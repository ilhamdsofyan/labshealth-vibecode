...
class MasterDataCleansingService
{
    public function stats(): array
    {
        return [
            'combined_diseases' => $this->combinedCount('diseases'),
            'combined_medications' => $this->combinedCount('medications'),
            'duplicate_disease_keys' => $this->duplicateKeyCount('diseases'),
            'duplicate_medication_keys' => $this->duplicateKeyCount('medications'),
        ];
    }

    public function cleanse(): array
    {
    ...
    protected function combinedCount(string $table): int
    {
    ...
    protected function duplicateKeyCount(string $table): int
    {
        return DB::table($table)
            ->selectRaw('LOWER(TRIM(name)) as k, COUNT(*) c')
            ->groupBy('k')
            ->having('c', '>', 1)
            ->count();
    }
