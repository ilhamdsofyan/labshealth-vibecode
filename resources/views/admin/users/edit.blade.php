                ...
                <div class="col-md-6">
                    ...
                    <select name="employee_id" class="form-select">
                        ...
                        @foreach($employees as $employee)
                            <option value="{{ $employee->id }}" {{ (string) old('employee_id', $user->employee_id) === (string) $employee->id ? 'selected' : '' }}>
                                {{ $employee->name }}{{ $employee->nip ? ' - ' . $employee->nip : '' }}{{ $employee->role_type ? ' (' . ucfirst($employee->role_type) . ')' : '' }}
                            </option>
                        @endforeach
                    </select>
                    <div class="form-text">Avatar user tetap terpisah. Jika avatar user kosong, sistem akan memakai avatar dari master pegawai sebagai default.</div>
